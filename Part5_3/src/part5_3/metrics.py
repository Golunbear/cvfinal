"""Evaluation metrics for Part 5.3 outputs."""
...
def evaluate_sequence(
    ...
    if max_frames and max_frames > 0:
        pred_paths = pred_paths[:max_frames]
        gt_paths = gt_paths[:max_frames]
    if not pred_paths:
    ...
    if not pairs:
        pairs = list(zip(pred_paths[: min(len(pred_paths), len(gt_paths))], gt_paths[: min(len(pred_paths), len(gt_paths))]))
    if not pairs:
