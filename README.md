...
## Part 1: Baseline Approaches

Part 1 lives in `Part5_1/baseline/` and implements hand-crafted baseline methods for video super-resolution:

- `Bicubic` and `Lanczos` interpolation baselines (spatial-only)
- `SRCNN` (3-layer CNN single-frame baseline)
- Temporal weighted averaging baseline
- Optional unsharp masking for post-processing
...
Repeat for each method to prepare demo videos.

### Notes

- Evaluation assumes predicted frames and GT frames share the same filenames.
- If evaluating real-world clips without GT, use qualitative comparison and report separately.
